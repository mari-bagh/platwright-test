...
let role_id: string;
test.describe.skip('"manage space" permissions disablement', async () => {
    test(`verify "Add space" button's absence for users without permission`, async ({ context, page }) => {
        ...
        await expect(addSpace).not.toBeVisible();
    });
